...
bool NavigateToTopologyNavigator::configure(rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node,
                                            std::shared_ptr<nav2_util::OdomSmoother> odom_smoother) {
  start_time_ = rclcpp::Time(0);
  auto node = parent_node.lock();

  if (!node->has_parameter("start_blackboard_id")) {
    node->declare_parameter("start_blackboard_id", "start_vertex_id");
  }
  if (!node->has_parameter("end_blackboard_id")) {
    node->declare_parameter("end_blackboard_id", "end_vertex_id");
  }
  ...
  end_blackboard_id_ = node->get_parameter("end_blackboard_id").as_string();
...
std::string NavigateToTopologyNavigator::getDefaultBTFilepath(rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node) {
...
bool NavigateToTopologyNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal) {
  auto bt_xml_filename = goal->behavior_tree;

  if (!bt_action_server_->loadBehaviorTree(bt_xml_filename)) {
    RCLCPP_ERROR(logger_, "Error loading XML file: %s. Navigation canceled.", bt_xml_filename.c_str());
    return false;
  }
...
void NavigateToTopologyNavigator::goalCompleted(typename ActionT::Result::SharedPtr /*result*/,
                                                const nav2_behavior_tree::BtStatus /*final_bt_status*/) {}

void NavigateToTopologyNavigator::onLoop() {
  ...
  nav2_util::getCurrentPose(current_pose, *feedback_utils_.tf, feedback_utils_.global_frame,
  ...
  try {
    ...
    auto find_closest_pose_idx = [&current_pose, &current_path]() {
      ...
      for (size_t curr_idx = 0; curr_idx < current_path.poses.size(); ++curr_idx) {
        double curr_dist = nav2_util::geometry_utils::euclidean_distance(current_pose, current_path.poses[curr_idx]);
    ...
    double distance_remaining = nav2_util::geometry_utils::calculate_path_length(current_path, find_closest_pose_idx());
    ...
    if ((std::abs(current_linear_speed) > 0.01) && (distance_remaining > 0.1)) {
      estimated_time_remaining = rclcpp::Duration::from_seconds(distance_remaining / std::abs(current_linear_speed));
    }
...
void NavigateToTopologyNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal) {
...
void NavigateToTopologyNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal) {
  auto start_vertex_id{goal->start_vertex_id};
  auto end_vertex_id{goal->end_vertex_id};

  RCLCPP_INFO(logger_, "StartNode: %d, EndNode: %d", start_vertex_id, end_vertex_id);
  // if (goal->poses.size() > 0) {
  //   RCLCPP_INFO(logger_, "Begin navigating from current location through %zu poses to (%.2f, %.2f)",
  //   goal->poses.size(),
  ...
  blackboard->set<int>(start_blackboard_id_, start_vertex_id);
  blackboard->set<int>(end_blackboard_id_, end_vertex_id);
}
} // namespace nav2_bt_navigator
