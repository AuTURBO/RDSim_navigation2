...
BtNavigator::BtNavigator(rclcpp::NodeOptions options)
    : nav2_util::LifecycleNode("bt_navigator", "", options.automatically_declare_parameters_from_overrides(true)) {
  ...
  declare_parameter_if_not_declared(this, "odom_topic", rclcpp::ParameterValue(std::string("odom")));
}
...
nav2_util::CallbackReturn BtNavigator::on_configure(const rclcpp_lifecycle::State & /*state*/) {
  ...
  topology_navigator_ = std::make_unique<nav2_bt_navigator::NavigateToTopologyNavigator>();
  ...
  if (!pose_navigator_->on_configure(shared_from_this(), plugin_lib_names, feedback_utils, &plugin_muxer_,
                                     odom_smoother_)) {
    return nav2_util::CallbackReturn::FAILURE;
  }

  if (!poses_navigator_->on_configure(shared_from_this(), plugin_lib_names, feedback_utils, &plugin_muxer_,
                                      odom_smoother_)) {
    return nav2_util::CallbackReturn::FAILURE;
  }

  if (!topology_navigator_->on_configure(shared_from_this(), plugin_lib_names, feedback_utils, &plugin_muxer_,
                                         odom_smoother_)) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
nav2_util::CallbackReturn BtNavigator::on_activate(const rclcpp_lifecycle::State & /*state*/) {
  ...
  if (!poses_navigator_->on_activate() || !pose_navigator_->on_activate() || !topology_navigator_->on_activate()) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
nav2_util::CallbackReturn BtNavigator::on_deactivate(const rclcpp_lifecycle::State & /*state*/) {
  ...
  if (!poses_navigator_->on_deactivate() || !pose_navigator_->on_deactivate() ||
      !topology_navigator_->on_deactivate()) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
nav2_util::CallbackReturn BtNavigator::on_cleanup(const rclcpp_lifecycle::State & /*state*/) {
  ...
  if (!poses_navigator_->on_cleanup() || !pose_navigator_->on_cleanup() || !topology_navigator_->on_cleanup()) {
    return nav2_util::CallbackReturn::FAILURE;
  }

  poses_navigator_.reset();
  pose_navigator_.reset();
  topology_navigator_.reset();
